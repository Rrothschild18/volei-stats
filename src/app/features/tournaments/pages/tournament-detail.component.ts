    ...
    <div class="p-4 max-w-4xl mx-auto">
      @if (tournament()) {
        <div class="flex justify-between items-center mb-4">
          ...
          <span [class]="tournament()!.status === 'completed' ? 'text-green-600 font-medium' : 'text-blue-600 font-medium'">
            {{ tournament()!.status === 'completed' ? 'Finalizado' : 'Em andamento' }}
          </span>
        </div>

        <p class="text-gray-600 mb-4">{{ tournament()!.teams.length }} duplas • Limite: {{ tournament()!.pointLimit }} pontos</p>
              ...
              <mat-card>
                <mat-card-content class="p-3">
                  <div class="flex justify-between items-center">
                    <div>
                      <span class="text-xs text-gray-500 uppercase">{{ getPhaseLabel(match.phase) }}</span>
                      <p class="font-medium">
            ...
            @for (match of pendingMatches(); track $index) {
              <mat-card class="border-l-4 border-blue-400">
                <mat-card-content class="p-4">
                  <span class="text-xs text-gray-500 uppercase block mb-2">{{ getPhaseLabel(match.phase) }}</span>
                  <p class="font-medium mb-3">{{ getTeamNames(match.teamAId) }} vs {{ getTeamNames(match.teamBId) }}</p>

                  <div class="flex items-center gap-3">
                    <mat-form-field appearance="outline" class="w-20">
                      <mat-label>Placar A</mat-label>
                      <input matInput type="number" min="0" [(ngModel)]="scoreInputs[$index].scoreA" />
                    </mat-form-field>
                    <span class="font-bold">x</span>
                    <mat-form-field appearance="outline" class="w-20">
                      <mat-label>Placar B</mat-label>
                      <input matInput type="number" min="0" [(ngModel)]="scoreInputs[$index].scoreB" />
                    </mat-form-field>
                    <button mat-raised-button (click)="saveMatch($index, match)">
                      Salvar
                    </button>
                  </div>
        ...
        @if (tournament()!.status === 'completed' && tournament()!.finalStandings.length > 0) {
          ...
          <div class="flex flex-col gap-1">
            @for (standing of tournament()!.finalStandings; track standing.teamId) {
              <div class="flex items-center gap-2 p-2 rounded" [class]="standing.position === 1 ? 'bg-yellow-50' : standing.position === 2 ? 'bg-gray-50' : ''">
...
export class TournamentDetailComponent implements OnInit {
  ...
  async loadTournament(id: string) {
    ...
    if (tournament.status !== 'completed') {
      ...
      if (matches.filter(m => m.winnerId).length === 0) {
        pending = this.tournamentService.generateInitialMatches(tournament.teams);
      } else {
  ...
  async saveMatch(index: number, bracketMatch: BracketMatch) {
    ...
    if (winnerScore < tournament.pointLimit) {
      this.matchWarnings[index] = `Aviso: O placar do vencedor (${winnerScore}) é menor que o limite configurado (${tournament.pointLimit}).`;
    }
    ...
    if (this.tournamentService.isTournamentComplete(tournament, allMatches)) {
      ...
      tournament.finalStandings = this.tournamentService.computeFinalStandings(tournament, allMatches);
    }
